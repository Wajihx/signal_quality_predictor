...
# User inputs
rsrp = st.slider("RSRP (dBm)", min_value=-120, max_value=-70, value=-95)
rsrq = st.slider("RSRQ (dB)", min_value=-20, max_value=-3, value=-10)
sinr = st.slider("SINR (dB)", min_value=-10, max_value=30, value=10)
rssi = st.slider("RSSI (dBm)", min_value=-100, max_value=-50, value=-75)
throughput = st.slider("Data Throughput (Mbps)", min_value=0.0, max_value=100.0, value=30.0, step=0.1)
latitude = st.number_input("Latitude (Tunisia approx. 34 to 36)", min_value=34.0, max_value=36.0, value=35.0, format="%.6f")
longitude = st.number_input("Longitude (Tunisia approx. 9 to 11)", min_value=9.0, max_value=11.0, value=10.0, format="%.6f")
